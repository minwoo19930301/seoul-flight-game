...
self.onmessage=async({data})=>{
  const {id,type}=data;
  try{
    if(type==='init'){terrain=data.terrain;keepouts=Array.isArray(data.keepouts)?data.keepouts:[];self.postMessage({id,ready:true});return;}
    ...
    const result=buildCityGeometry(tile,terrain,data.tile.lod,keepouts);
    inflight.delete(url);
